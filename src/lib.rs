...
#[derive(Debug)]
struct RthybridHindmarshRose1984NeuronRust {
    input_syn: f64,
...
impl Default for RthybridHindmarshRose1984NeuronRust {
    fn default() -> Self {
        Self {
            input_syn: 0.0,
...
impl RthybridHindmarshRose1984NeuronRust {
...
impl PluginDescriptor for RthybridHindmarshRose1984NeuronRust {
    fn name() -> &'static str {
...
impl PluginRuntime for RthybridHindmarshRose1984NeuronRust {
    ...
    fn process_tick(&mut self, _tick: u64, period_seconds: f64) {
        ...
        for _ in 0..steps {
            ...
            rk4_step(&mut vars, self.dt, |st, der| {
                der[0] = st[1] + b * st[0] * st[0] - a * st[0] * st[0] * st[0] - st[2] + i - input_syn;
                der[1] = c - d * st[0] * st[0] - st[1];
                der[2] = r * (s * (st[0] - xr) - st[2]);
            });
...
rtsyn_plugin::export_plugin!(RthybridHindmarshRose1984NeuronRust);
