#include "rthybrid_hindmarsh_rose_1984_neuron_c.h"
#include <float.h>
#include <math.h>
#include <string.h>

enum { DT=0, B=1, A=2, I=3, SYN=4, C=5, D=6, R=7, S=8, XR=9 };

static void rthybrid_hindmarsh_rose_1984_neuron_c_update_burst_settings(rthybrid_hindmarsh_rose_1984_neuron_c_state_t *s);
static int key_eq(const char *key, size_t len, const char *lit);
static double rthybrid_hindmarsh_rose_1984_neuron_c_select_optimal_dt(double pts_match);
...
typedef struct {
  ...
  double xr;
  double i;
} rthybrid_hindmarsh_rose_1984_neuron_c_ctx_t;

static void rthybrid_hindmarsh_rose_1984_neuron_c_deriv(const double *vars, double *deriv, size_t n, void *user_data) {
  (void)n;
  if (vars == NULL || deriv == NULL || user_data == NULL) return;
  const rthybrid_hindmarsh_rose_1984_neuron_c_ctx_t *ctx =
      (const rthybrid_hindmarsh_rose_1984_neuron_c_ctx_t *)user_data;
  deriv[0] = vars[1] + ctx->b * vars[0] * vars[0] - ctx->a * vars[0] * vars[0] * vars[0] - vars[2] + ctx->i - ctx->input_syn;
  deriv[1] = ctx->c - ctx->d * vars[0] * vars[0] - vars[1];
  deriv[2] = ctx->r * (ctx->s * (vars[0] - ctx->xr) - vars[2]);
}

void rthybrid_hindmarsh_rose_1984_neuron_c_init(rthybrid_hindmarsh_rose_1984_neuron_c_state_t *s) {
  memset(s, 0, sizeof(*s));
...
void rthybrid_hindmarsh_rose_1984_neuron_c_set_config(rthybrid_hindmarsh_rose_1984_neuron_c_state_t *s, const char *key, size_t len, double value) {
  ...
  rthybrid_hindmarsh_rose_1984_neuron_c_update_burst_settings(s);
}

void rthybrid_hindmarsh_rose_1984_neuron_c_set_input(rthybrid_hindmarsh_rose_1984_neuron_c_state_t *s, const char *name, size_t len, double value) {
...
void rthybrid_hindmarsh_rose_1984_neuron_c_process(rthybrid_hindmarsh_rose_1984_neuron_c_state_t *s, double period_seconds) {
  ...
  rthybrid_hindmarsh_rose_1984_neuron_c_ctx_t ctx = {
  ...
  for (size_t step = 0; step < steps; ++step) {
    ...
    rtsyn_plugin_rk4_step_n(s->vars, 3, s->params[DT], rthybrid_hindmarsh_rose_1984_neuron_c_deriv, &ctx);
    if (!isfinite(s->vars[0]) || !isfinite(s->vars[1]) || !isfinite(s->vars[2])) {
      s->vars[0] = prev[0];
      s->vars[1] = prev[1];
      s->vars[2] = prev[2];
...
static void rthybrid_hindmarsh_rose_1984_neuron_c_update_burst_settings(rthybrid_hindmarsh_rose_1984_neuron_c_state_t *s) {
  ...
  s->params[DT] = rthybrid_hindmarsh_rose_1984_neuron_c_select_optimal_dt(pts_burst);
...
double rthybrid_hindmarsh_rose_1984_neuron_c_get_output(const rthybrid_hindmarsh_rose_1984_neuron_c_state_t *s, const char *key, size_t len) {
